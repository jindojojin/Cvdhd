...
$(document).ready(function () {
    $("#submit_btn").click(async function () {
        ...
        if(validateCowInfo(cowInfo)){
            console.log(cowInfo);
            var response = await HTTP.post('/addCow',cowInfo) // Đẩy thông tin lên server tập trung
...
},{"../HttpControler":2,"autocomplete-js":3}],2:[function(require,module,exports){
const myServerUrl = "http://localhost:9000";
module.exports = {
    get: function (uri) {
        fetch(myServerUrl + uri)
            .then(data => data.json())
    ...
    post: function (uri, object) {
        ...
        fetch(myServerUrl + uri, postPram)
            .then(data => data.json())
            .then(res => console.log(res))
